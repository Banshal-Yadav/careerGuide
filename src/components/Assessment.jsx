...
import { CheckCircle2, XCircle, Target, TrendingUp, Lightbulb, Briefcase, BarChartHorizontal, Zap, Clock, IndianRupee, Award } from 'lucide-react';
...
const AnalysisResults = ({ analysis, onRetry }) => {
  ...
  return (
    <div className="results-container">
      <div className="results-header"><h2 className="success-title">Your Personalized Career Roadmap</h2><p className="results-subtitle">Powered by Gemini</p></div>
        ...
        {analysis.careerAnalysis.map((career, index) => {
          ...
          return (
            <div key={career.title} className="results-card career-card-detailed active-career">
              <div className="career-title-header"><h4><Briefcase size={28} /> {career.title}</h4><RadialProgressBar score={career.matchScore} /></div>
              <div className="details-section"><h5><BarChartHorizontal size={16} /> Job Outlook</h5><p className="job-outlook">{career.jobOutlook}</p></div>

              <div className="details-section">
                <h5><Clock size={16} /> A Day in the Life</h5>
                <p>{career.dayInTheLife}</p>
              </div>
              ...
              <div className="details-section">
                <h5><IndianRupee size={16} /> Salary (Entry-Level)</h5>
              ...
              <div className="details-section">
                <h5><Award size={16} /> Key Certifications</h5>
                <ul className="skill-gaps-list">{career.suggestedCertifications?.map((cert, i) => <li key={i}>{cert}</li>)}</ul>
              </div>
              
              <div className="learning-path"><p><Lightbulb size={80} className="icon-yellow" /> <strong>Your First Step:</strong> {career.suggestedLearningPath}</p></div>
            </div>
          );
        })}
      </div>
...
const Assessment = () => {
  ...
  const suggestedSkills = [...skillsData.technical.slice(0, 6), ...skillsData.soft.slice(0, 4), ...skillsData.industry.slice(0, 4)].map(skill => skill.name);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const assessmentData = { userId: user.uid, userEmail: user.email, rawSkills: skills, rawInterests: interests, education: education, matchedSkills: matchedSkills, aiSummary: analysisResult.summary, aiStrengths: analysisResult.strengths, aiGrowthAreas: analysisResult.growthAreas, aiCareerAnalysis: analysisResult.careerAnalysis, createdAt: new Date() };
      const docRef = await addDoc(collection(db, 'assessments'), assessmentData);
      const profileRef = doc(db, 'profiles', user.uid);
      const profileSnap = await getDoc(profileRef);

      if (profileSnap.exists()) {
        await updateDoc(profileRef, { assessments: arrayUnion({ ...assessmentData, assessmentId: docRef.id }) });
      } else {
        await setDoc(profileRef, { userEmail: user.email, assessments: [{ ...assessmentData, assessmentId: docRef.id }] });
      }
    } catch (err) {
  ...
  if (aiAnalysis) {
    return (
      <div className="assessment-container">
        <AnalysisResults 
          analysis={aiAnalysis} 
          onRetry={() => { setAiAnalysis(null); setSkills(''); setInterests(''); setEducation('undergraduate'); setError(''); }} 
        />
      </div>
    );
  }
  ...
  return (
    <div className="assessment-container">
      ...
      <form onSubmit={handleSubmit} className="assessment-form">
        <div className="form-group"><label className="form-label">Your Skills</label><input type="text" className="form-input" placeholder="Type your skills or select from the options below" value={skills} onChange={(e) => setSkills(e.target.value)} /><div className="skill-chips-container">{suggestedSkills.map(skill => (<button type="button" key={skill} className="skill-chip" onClick={() => handleSkillChipClick(skill)}>{skill}</button>))}</div></div>
        <div className="form-group"><label className="form-label">Your Interests based on your Skills</label><input type="text" className="form-input" placeholder="e.g., Building web apps, AI, Video Games" value={interests} onChange={(e) => setInterests(e.target.value)} /></div>
        <div className="form-group"><label className="form-label">Highest Education Level</label><select value={education} onChange={(e) => setEducation(e.target.value)} className="form-select"><option value="12th">12th Grade / High School</option><option value="diploma">Diploma</option><option value="undergraduate">Undergraduate Degree</option><option value="postgraduate">Postgraduate Degree</option><option value="other">Other</option></select></div>
