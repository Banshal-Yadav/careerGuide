...
  return careersData
    .map(career => {
      // lists all required skills for the career
      const allRequiredSkillIds = career.skillRequirements.map(req => req.skillId);
      
      // count how many user skills match what the career needs
      const matchCount = userSkillIds.filter(skillId => allRequiredSkillIds.includes(skillId)).length;
      
      return { ...career, matchCount };
    })
    .filter(career => career.matchCount > 0) // only show careers with at least one matching skill
    .sort((a, b) => b.matchCount - a.matchCount) // sort by the highest number of matches
    .slice(0, 15); // just show the top 15 results
};